"""
Nature-family figure styling for Project 3 plots (shared by figures/make_publication_figure.py
and figures/make_generation_funnel_figure.py).
...
# Free to diverge from project1_screening / project2_mlip_md; does not need
# to match the other two repos.
# =============================================================================

# ── Project 3 semantic palette: generate -> screen -> S.U.N. status ──────────
C_SUN = PALETTE["blue_main"]     # stable + unique + novel (the hero signal)
C_STABLE = "#8FB2D6"             # stable but not S.U.N. (lighter blue)
C_UNSTABLE = "#BEBEBE"           # screened out (neutral grey; distinct from PALETTE
                                  # neutral_light #CFCECE -- kept for exact parity with
                                  # the already-published fig_inverse_design.png)
C_CUT = PALETTE["red_strong"]    # stability cutoff (reference cue)
C_TARGET = PALETTE["blue_main"]  # faint "kept" region shade
